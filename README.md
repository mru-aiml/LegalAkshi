...
- Providers (`backend/app/services/vision/`): `groq` (active demo path — `qwen/qwen3.8-27b` via OpenAI-compatible chat/completions, server-side key), `openrouter`, `gemini` and `mock` (deterministic scripted candidates for tests). Resolution order per provider is documented in `backend/.env.example`; keys never leave the backend. Calls stay bounded (max 6 per inspection, 30s per call, 100s overall, per-inspection cache).
